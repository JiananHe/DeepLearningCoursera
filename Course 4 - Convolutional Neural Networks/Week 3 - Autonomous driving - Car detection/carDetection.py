...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold=.6):
    ...
    box_class_scores = K.max(box_scores, axis=-1, keepdims=False)  # (19, 19, 5, 1)

    # Create a filtering mask based on "box_class_scores" by using "threshold". The mask should have the
    # same dimension as box_class_scores, and be True for the boxes you want to keep (with probability >= threshold)
    filtering_mask = K.greater_equal(box_class_scores, threshold)  # (19, 19, 5, 1)
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes=10, iou_threshold=0.5):
    ...
    # Use K.gather() to select only nms_indices from scores, boxes and classe
